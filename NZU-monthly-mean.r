...
str(data)
'data.frame':	2225 obs. of  5 variables:
...
## weekly time series
str(data$week) 
Class 'aweek'  atomic [1:2225] 2010-W19-5 2010-W20-5 2010-W21-6 2010-W23-5 ...
  ..- attr(*, "week_start")= int 1 
...
‘zoo’ series from 2010-05-10 to 2025-10-13
  Data: num [1:832] 17.8 17.5 17.5 NA 17 ...
  Index:  Date[1:832], format: "2010-05-10" "2010-05-17" "2010-05-24" "2010-05-31" ...
...
‘zoo’ series from 2010-05-10 to 2025-12-15
  Data: num [1:832] 17.8 17.5 17.5 17.2 17 ...
...
'data.frame':	832 obs. of  2 variables:
...
par(mar=c(2.7,2.7,1,1)+0.1)
plot(spotpricefilleddataframe[["date"]],spotpricefilleddataframe[["price"]],tck=0.01,ann=TRUE, las=1,col= "#B80000" ,lwd=1,type='l',lty=1)
grid(col="darkgray",lwd=1)
axis(side=4, tck=0.01, las=0,tick=TRUE,labels = FALSE)
...
par(mar=c(2.7,2.7,1,1)+0.1)
plot(weeklypricefilled,tck=0.01,ann=T, las=1,col="#B80000",lwd=1,type='l',lty=1)
grid(col="darkgray",lwd=1)
axis(side=4, tck=0.01, las=0,tick=TRUE,labels = FALSE)
mtext(side=1,cex=0.8,line=-1.1,"Data: 'NZU monthly prices' https://github.com/theecanmole/nzu")
