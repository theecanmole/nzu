...
# R code to create a mean monthly time series vector of spot prices of the New Zealand Emission Unit (NZU) from raw data of irregular prices webscraped from various web sources.
...
# examine dataframe
str(data)
'data.frame':	2290 obs. of  5 variables:
...
str(data)
'data.frame':	2289 obs. of  5 variables:
...
# examine the monthly price dataframe
str(monthprice)
...
## weekly time series
str(data$week) 
 'aweek' chr [1:2289] "2010-W19-5" "2010-W20-5" "2010-W21-6" "2010-W23-5" ...
 - attr(*, "week_start")= int 1
...
'data.frame':	748 obs. of  2 variables:
 $ week :Class 'aweek'  atomic [1:707] 2010-W19 2010-W20 2010-W21 2010-W23 ...
  .. ..- attr(*, "week_start")= int 1
 $ price: num  17.8 17.5 17.5 17 17.8 ...
...
str(weeklypriceallDates)
 Date[1:849], format: "2010-05-10" "2010-05-17" "2010-05-24" "2010-05-31" ...
...
‘zoo’ series from 2010-05-10 to 2026-04-27
  Data: num [1:849] 17.8 17.5 17.5 NA 17 ...
  Index:  Date[1:849], format: "2010-05-10" "2010-05-17" "2010-05-24" "2010-05-31" "2010-06-07" ...
...
‘zoo’ series from 2010-05-10 to 2026-06-08
  Data: num [1:849] 17.8 17.5 17.5 17.2 17 ...
  Index:  Date[1:849], format: "2010-05-10" "2010-05-17" "2010-05-24" "2010-05-31" "2010-06-07" ...
...
'data.frame':	849 obs. of  2 variables:
...
summary(spotprice$date)
 NAs 0


# How many day dates should be included if there were prices for all days from May 2010 to the present?
spotpricealldates <- seq.Date(from=min(spotprice$date), to=max(spotprice$date), by="day")

#spotpricealldates <- seq.Date(spotprice$date[1], spotprice[["date"]][2060], by= "day")

length(spotpricealldates) 
[1] 5936
# how many days with missing values are there?
...
'data.frame':	5936 obs. of  2 variables:
...
‘zoo’ series from 2010-05-14 to 2026-08-13
  Data: num [1:5936] 17.8 NA NA NA NA ...
  Index:  Date[1:5936], format: "2010-05-14" "2010-05-15" "2010-05-16" "2010-05-17" "2010-05-18" ...
...
‘zoo’ series from 2010-05-14 to 2026-06-12
  Data: num [1:5936] 17.8 17.7 17.7 17.6 17.6 ...
  Index:  Date[1:5936], format: "2010-05-14" "2010-05-15" "2010-05-16" "2010-05-17" "2010-05-18" ...
...
# read in holidays from qlcal package
library("qlcal")
citation("qlcal")
To cite package ‘qlcal’ in publications use:
...
# create logical vector for days that were holidays
idHoliday <- spotpricefilleddataframe$businessday == "FALSE"
table(idHoliday)
idHoliday
FALSE  TRUE
 4059   181
# omit the the holiday dates with the logical vector
spotpricefilleddataframe <- spotpricefilleddataframe[!idHoliday, ]

str(spotpricefilleddataframe)  
'data.frame':	4059 obs. of  4 variables:
 $ date       : Date, format: "2010-05-14" "2010-05-17" ...
 $ price      : num  17.8 17.6 17.6 17.6 17.5 ...
 $ day        : chr  "Friday" "Monday" "Tuesday" "Wednesday" ...
 $ businessday: logi  TRUE TRUE TRUE TRUE TRUE TRUE ...
...
str(spot) 
'data.frame':	4059 obs. of  2 variables:
 $ date : Date, format: "2010-05-14" "2010-05-17" ...
 $ price: num  17.8 17.6 17.6 17.6 17.5 ...
...
# summarise
summary(spotrollmean31[["price"]])
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NAs
   1.75    9.41   21.37   30.36   52.05   86.95      20
# ok 10 NAs at end and start
...
# #B80000				Guardsman Red,  old colour #ED1A3B
...
plot(monthprice,tck=0.01,ann=TRUE, las=1,col="#B80000",lwd=2,type='l',lty=1)
grid(col="darkgray",lwd=1)
axis(side=4, tck=0.01, las=0,tick=TRUE,labels = FALSE)
mtext(side=1,cex=0.8,line=-1.1,"Data: 'NZU monthly prices' https://github.com/theecanmole/nzu")
mtext(side=3,cex=1.2, line=-2.2,expression(paste("New Zealand Unit monthly mean prices 2010 - 2026")) )
...
par(mar=c(2.7,2.7,1,1)+0.1)
plot(spotpricefilleddataframe[["date"]],spotpricefilleddataframe[["price"]],xlab="",ylab="",tck=0.01,ann=TRUE, las=1,col= "#B80000" ,lwd=1,type='l',lty=1)
grid(col="darkgray",lwd=1)
axis(side=4, tck=0.01, las=0,tick=TRUE,labels = FALSE)
mtext(side=1,cex=0.9,line=-1.3,"Data: 'NZU prices' https://github.com/theecanmole/nzu")
mtext(side=3,cex=1.2, line=-2.2,expression(paste("New Zealand Unit spot prices 2010 - 2026")) )
mtext(side=2,cex=1, line=-1.3,"$NZ Dollars/tonne")
mtext(side=4,cex=0.75, line=0.05,R.version.string)
dev.off()
...
par(mar=c(2.7,2.7,1,1)+0.1)
plot(weeklypricefilled,xlab="",ylab="",tck=0.01,ann=T, las=1,col="#B80000",lwd=1,type='l',lty=1)
grid(col="darkgray",lwd=1)
axis(side=4, tck=0.01, las=0,tick=TRUE,labels = FALSE)
mtext(side=1,cex=0.8,line=-1.1,"Data: 'NZU monthly prices' https://github.com/theecanmole/nzu")
mtext(side=3,cex=1.2, line=-2.2,expression(paste("New Zealand Unit mean weekly spot prices 2010 2026")) )
mtext(side=2,cex=1, line=-1.3,"$NZ Dollars/tonne")
mtext(side=4,cex=0.75, line=0.05,R.version.string)
dev.off()

# 	colour 		Jelly Bean blue  "#2D74B5"
...
par(mar=c(2.7,2.7,1,1)+0.1)
plot(spotrollmean31,xlab="",ylab="",tck=0.01,ann=T, las=1,col="#2D74B5",lwd=2,type='l',lty=1)
grid(col="darkgray",lwd=1)
axis(side=4, tck=0.01, las=0,tick=TRUE,labels = FALSE)
mtext(side=1,cex=0.8,line=-1.1,"Data: 'NZU prices' https://github.com/theecanmole/nzu")
mtext(side=3,cex=1.2, line=-2.2,expression(paste("New Zealand Unit 21 day rolling mean spot prices 2010 2026")) )
mtext(side=2,cex=1, line=-1.3,"$NZ Dollars/tonne")
mtext(side=4,cex=0.75, line=0.05,R.version.string)
dev.off()
...
[1] qlcal_0.1.1  zoo_1.8-15   aweek_1.0.3  rkward_0.8.0

loaded via a namespace (and not attached):
[1] compiler_4.5.3 tools_4.5.3    Rcpp_1.1.2     grid_4.5.3     lattice_0.22-9
# to 9/07/2026
sessionInfo()
...
other attached packages:
[1] qlcal_0.0.17 zoo_1.8-15   aweek_1.0.3  rkward_0.7.5

loaded via a namespace (and not attached):
[1] compiler_4.2.2 tools_4.2.2    Rcpp_1.1.1     grid_4.2.2     lattice_0.22-7
...
other attached packages:
[1] qlcal_0.0.17 zoo_1.8-12   aweek_1.0.3  rkward_0.7.5

loaded via a namespace (and not attached):
[1] compiler_4.4.2 tools_4.4.2    Rcpp_1.0.14    grid_4.4.2     lattice_0.22-6
# to 23/12/2025
sessionInfo()
...
other attached packages:
[1] zoo_1.8-14   aweek_1.0.3  rkward_0.7.0
